...
COLORREF CHyperLink::g_redVisitedColor  = RGB( 0,  0, 255);   // Purple;
...
BOOL CHyperLink::ConvertStaticToHyperlink(HWND hwndCtl, LPCTSTR strURL)
{
	...
	/*DWORD*/LONG_PTR dwStyle = GetWindowLongPtr(hwndCtl, GWL_STYLE);
...
BOOL CHyperLink::setURL(LPCTSTR strURL)
{
	...
	StringCchCopy(m_strURL, lstrlen(strURL)+1, strURL);
	...
	case WM_MOUSEMOVE:
		{
			...
			else
			{
				...
				SetTextColor(hdc, CHyperLink::g_redVisitedColor);
				//SendMessage
